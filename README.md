...
Prompt behavior should be treated as experimentally sensitive rather than fixed. Vision-language outputs can shift with prompt wording, decoding settings, model family, and the visual encoder used by a given provider. In transformer-based vision systems, image evidence is often represented through patch-level tokens, so changes in patch resolution, visual tokenization, or cross-modal alignment can affect which objects, attributes, colors, and spatial relations are emphasized. This is one reason maverick-oss keeps agents, prompts, and models configurable: different LLM/VLM backends may exhibit different concept associations, spatial-reasoning limitations, and grounding behavior, as shown in recent work on concept association bias ([Yamada et al., 2024](https://doi.org/10.48550/arXiv.2212.12043)) and spatial reasoning in vision-language models ([Kamath et al., 2023](https://doi.org/10.18653/v1/2023.emnlp-main.568)).
